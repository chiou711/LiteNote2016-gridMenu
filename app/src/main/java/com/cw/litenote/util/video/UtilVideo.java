    	...
    	if(hasVideoExtension(mPictureString,mAct))
	  	{
        	...
        	if(!hasMediaControlWidget) {
				...
				ui.setVideoViewUI(strPicture,position);
				ui.delay_pagerUI_all_off(3006,strPicture, Note.mCurrentPosition); // for 3 seconds, _showPictureViewUI
			}
        	else
	...
	static void setVideoPlayerListeners(final ViewPager pager,final String picString)
	{
		...
		mVideoView.setOnPreparedListener(new OnPreparedListener() 
		{
			...
			public void onPrepared(MediaPlayer mp) 
			{
				...
				{
					Note_UI.updateVideoPlayButtonState(pager,mPlayVideoPosition);
					Note_UI.primaryVideoSeekBarProgressUpdater(pager,UtilVideo.mPlayVideoPosition,picString);
